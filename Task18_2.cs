...
1,6
2,0", 6, "6,1")]
    [TestCase(@"Task18.txt", 70, "50,28")]
    public void Task(string input, int max, string expected)
    {
        input = File.Exists(input) ? File.ReadAllText(input) : input;

        var bots = new List<Point>();
        ...
        $"{result.Col},{result.Row}".Should().Be(expected);
    }
