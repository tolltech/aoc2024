...
292: 11 6 16 20",
        3749)]
    [TestCase(@"Task07.txt", 12839601725877L)]
    ...
    private long[] Operate(long[] ops, long currentResult, int currentIndex)
    {
        ...
        {
            return [currentResult + current, currentResult * current];
        }

        return Operate(ops, currentResult * current, currentIndex + 1).Concat(
                Operate(ops, currentResult + current, currentIndex + 1))
            .ToArray();
    }
}